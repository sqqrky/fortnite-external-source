...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const float square_sz = GetFrameHeight() * 0.8f; // Make the checkbox smaller (80% of original size)
    const ImVec2 pos = window->DC.CursorPos;
    ...
    // Adjust label's vertical position to align it with the middle of the checkbox, but outside the checkbox
    ImVec2 label_pos = ImVec2(check_bb.Max.x + style.ItemInnerSpacing.x, check_bb.Min.y + (check_bb.GetHeight() - label_size.y) * 0.5f);
    if (g.LogEnabled)
